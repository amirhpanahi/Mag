...
{
    public class NineNewsForMainPageViewComponent : ViewComponent
    {
        private readonly DataBaseContext _dbContext;
        public NineNewsForMainPageViewComponent(DataBaseContext dataBaseContex)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            var listNews = _dbContext.News.OrderByDescending(p => p.PublishNewsDate).Where(p => p.Status == StatusName.Publish).Take(9).ToList().Select(p => new NewsCardDto
            {
                Title = p.Title,
                Slug = p.Slug,
